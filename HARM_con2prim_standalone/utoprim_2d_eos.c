...
******************************************************************/
int Utoprim_2d_eos(FTYPE *U, struct of_geom *geom, FTYPE *prim, FTYPE *gamma, FTYPE *bsq)
{
...
//-faster   ncon[3] = -alpha * geom->gcon[0][3]; 
  ...
  // printf("Qsq = %.15e, Qdotn = %.15e, Qtsq = %.15e",Qsq,Qdotn,Qtsq);

  D = U[RHO] ;
  ye0=U[YE]/U[RHO];
  //fprintf(stderr," in 2d eos ye0 %e \n", ye0);
  ...
  // printf("gammasq = %23.15e | gamma = %23.15e\n",gammasq,gamma);
  ...
  // printf("P,T,w,W = %.15e,%.15e,%.15e,%.15e\n",p,temp_guess,w,W_last);
...
//  fflush(stdout);
  ...
  double LHS = W_last*W_last*W_last * ( W_last + 2.0*Bsq ) - QdotBsq*(2*W_last + Bsq);
  double RHS = W_last*W_last*(Qtsq-Bsq*Bsq);
  // printf("LHS = %.15e\nRHS = %.15e\nCondition satisfied? %d\n",LHS,RHS,LHS<=RHS);
  i_increase = 0;
  ...
  // printf("NR in : x_2d[0] = %.15e , x_2d[1] = %.15e\n",x_2d[0],x_2d[1]);
  
  retval = general_newton_raphson( x_2d, NEWT_DIM, D, ye0, &temp_guess, func_vsq ) ;

  // printf("NR out: x_2d[0] = %.15e , x_2d[1] = %.15e\n",x_2d[0],x_2d[1]);
...
//    //p = Qdotn+0.5*Bsq*(1.+vsq)-0.5*QdotBsq/(W*W)+W;
					   ...
					   FTYPE *,FTYPE , FTYPE, FTYPE *) )
{
  ...
  if( fabs(errx) <= NEWT_TOL ){
    ...
    fprintf(stderr,"error bad in 2deos %e \n",errx);
    fprintf(stderr, "x[0] %e\n", x[0] );
    fprintf(stderr, "x[1] %e\n", x[1] );
