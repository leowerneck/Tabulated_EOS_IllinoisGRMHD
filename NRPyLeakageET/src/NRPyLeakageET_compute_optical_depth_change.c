...
void NRPyLeakageET_compute_optical_depth_change(CCTK_ARGUMENTS, CCTK_REAL *l2norm) {
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;

  if(!NRPyLeakageET_ProcessOwnsData()) return;

  if(verbosity_level>1) CCTK_VInfo(CCTK_THORNSTRING,"Inside NRPyLeakageET_compute_optical_depth_change at Ref. Lev. %d",GetRefinementLevel(cctkGH));

  CCTK_REAL l2normL = 0.0;
#pragma omp parallel for reduction(+:l2normL)
  for(int k=cctk_nghostzones[2];k<cctk_lsh[2]-cctk_nghostzones[2];k++) {
    for(int j=cctk_nghostzones[1];j<cctk_lsh[1]-cctk_nghostzones[1];j++) {
      for(int i=cctk_nghostzones[0];i<cctk_lsh[0]-cctk_nghostzones[0];i++) {
        ...
        // Step 4: Add differences to local l2norm
        l2normL += diff_tau_0_nue*diff_tau_0_nue;
        l2normL += diff_tau_1_nue*diff_tau_1_nue;
        l2normL += diff_tau_0_anue*diff_tau_0_anue;
        l2normL += diff_tau_1_anue*diff_tau_1_anue;
        l2normL += diff_tau_0_nux*diff_tau_0_nux;
        l2normL += diff_tau_1_nux*diff_tau_1_nux;
      }
    }
  }

  // Step 5: Add local l2norm to output
  *l2norm += l2normL;

  if(verbosity_level>1) CCTK_VInfo(CCTK_THORNSTRING,"Local l2norm: %e | Accumulated at this level: %e",l2normL,*l2norm);
}
