...
void NRPyLeakageET_compute_GRMHD_source_terms_and_opacities_harm_constants(const CCTK_REAL rho_b,
  ...
  */
  ...
  const CCTK_REAL tmp_5 = 8*M_PI*NRPyLeakageET_harm_N_A*NRPyLeakageET_beta*rho_b_cgs*((3.0/8.0)*tmp_3 + 1.0/8.0)/NRPyLeakageET_harm_hc3;
  ...
  const CCTK_REAL tmp_21 = (1.0/((NRPyLeakageET_harm_hc3)*(NRPyLeakageET_harm_hc3)));
  ...
  const CCTK_REAL tmp_29 = ((M_PI)*(M_PI)*(M_PI))*pow(NRPyLeakageET_gamma_0, 6)*((tmp_24)*(tmp_24)*(tmp_24))*(tmp_25 + 1)*exp(-tmp_25)/NRPyLeakageET_harm_alpha_fs;
  ...
  const CCTK_REAL tmp_35 = NRPyLeakageET_harm_N_A*NRPyLeakageET_sigma_0*((T)*(T))*rho_b_cgs/((NRPyLeakageET_harm_m_e_c2)*(NRPyLeakageET_harm_m_e_c2));
  ...
  const CCTK_REAL tmp_47 = (3.0/2.0)*NRPyLeakageET_harm_hc3/(M_PI*NRPyLeakageET_harm_units_geom_to_cgs_L);
  ...
  *R_source = NRPyLeakageET_amu*NRPyLeakageET_harm_units_cgs_to_geom_R*(-tmp_32/(((tau_nue[0])*(tau_nue[0]))*tmp_32*tmp_36*tmp_48/tmp_46 + 1) + tmp_51/(((tau_anue[0])*(tau_anue[0]))*tmp_48*tmp_51*tmp_52/tmp_57 + 1));
  *Q_source = NRPyLeakageET_harm_units_cgs_to_geom_Q*(-tmp_69/(((tau_nue[1])*(tau_nue[1]))*tmp_69*tmp_70*tmp_72/tmp_75 + 1) - tmp_76/(((tau_anue[1])*(tau_anue[1]))*tmp_70*tmp_76*tmp_77/tmp_80 + 1) - 4*(NRPyLeakageET_C1pC2_nux_anux*NRPyLeakageET_enable_pair_nux_anux*tmp_61/((exp(tmp_18) + 1)*(exp(tmp_18) + 1)) + NRPyLeakageET_enable_plasmon_nux_anux*tmp_29*tmp_40*tmp_62/((exp(tmp_28) + 1)*(exp(tmp_28) + 1)) + tmp_58)/(((tau_nux[1])*(tau_nux[1]))*tmp_63*tmp_69*tmp_70/tmp_66 + 1));
  kappa_nue[0] = NRPyLeakageET_harm_units_geom_to_cgs_L*tmp_46;
  kappa_nue[1] = NRPyLeakageET_harm_units_geom_to_cgs_L*tmp_75;
  kappa_anue[0] = NRPyLeakageET_harm_units_geom_to_cgs_L*tmp_57;
  kappa_anue[1] = NRPyLeakageET_harm_units_geom_to_cgs_L*tmp_80;
  kappa_nux[0] = NRPyLeakageET_harm_units_geom_to_cgs_L*(tmp_34*tmp_35*tmp_81 + tmp_42*tmp_81);
  kappa_nux[1] = NRPyLeakageET_harm_units_geom_to_cgs_L*tmp_66;
}
