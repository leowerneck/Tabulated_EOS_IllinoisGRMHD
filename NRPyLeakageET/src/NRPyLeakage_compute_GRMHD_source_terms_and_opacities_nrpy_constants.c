...
void NRPyLeakage_compute_GRMHD_source_terms_and_opacities_nrpy_constants(const CCTK_REAL rho_b,
  ...
  const CCTK_REAL rho_b_cgs = rho_b * NRPyLeakage_units_geom_to_cgs_D;
...
*/
  ...
  const CCTK_REAL tmp_5 = 8*M_PI*NRPyLeakage_N_A*NRPyLeakage_beta*rho_b_cgs*((3.0/8.0)*tmp_3 + 1.0/8.0)/NRPyLeakage_hc3;
  ...
  const CCTK_REAL tmp_21 = (1.0/((NRPyLeakage_hc3)*(NRPyLeakage_hc3)));
  ...
  const CCTK_REAL tmp_29 = ((M_PI)*(M_PI)*(M_PI))*pow(NRPyLeakage_gamma_0, 6)*((tmp_24)*(tmp_24)*(tmp_24))*(tmp_25 + 1)*exp(-tmp_25)/NRPyLeakage_alpha_fs;
...
tmp_34 = (1 - Y_e)*((5.0/24.0)*tmp_3 + 1.0/24.0)/((2.0/3.0)*MAX(mu_n*tmp_0, 0) + 1);
  const CCTK_REAL tmp_35 = NRPyLeakage_N_A*NRPyLeakage_sigma_0*((T)*(T))*rho_b_cgs/((NRPyLeakage_m_e_c2)*(NRPyLeakage_m_e_c2));
...
tmp_41 = Y_e*((5.0/24.0)*tmp_3 + tmp_40)/((2.0/3.0)*MAX(mu_p*tmp_0, 0) + 1);
  ...
  const CCTK_REAL tmp_47 = (3.0/2.0)*NRPyLeakage_hc3/(M_PI*NRPyLeakage_units_geom_to_cgs_L);
