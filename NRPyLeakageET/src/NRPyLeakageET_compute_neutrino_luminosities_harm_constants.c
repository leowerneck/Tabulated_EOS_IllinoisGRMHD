...
void NRPyLeakageET_compute_neutrino_luminosities_harm_constants(const CCTK_REAL alpha,
  ...
  const CCTK_REAL tmp_9 = NRPyLeakageET_harm_N_A*NRPyLeakageET_sigma_0*((T)*(T))*rho_b_cgs/((NRPyLeakageET_harm_m_e_c2)*(NRPyLeakageET_harm_m_e_c2));
  ...
  const CCTK_REAL tmp_17 = M_PI/NRPyLeakageET_harm_hc3;
  ...
  const CCTK_REAL tmp_21 = 8*NRPyLeakageET_harm_N_A*NRPyLeakageET_beta*((T)*(T)*(T)*(T)*(T))*rho_b_cgs*tmp_17*((3.0/8.0)*tmp_11 + 1.0/8.0);
  ...
  const CCTK_REAL tmp_33 = (1.0/((NRPyLeakageET_harm_hc3)*(NRPyLeakageET_harm_hc3)));
  const CCTK_REAL tmp_34 = pow(T, 8);
  const CCTK_REAL tmp_36 = (1.0/3.0)*((M_PI)*(M_PI)*(M_PI))*NRPyLeakageET_beta*pow(NRPyLeakageET_gamma_0, 6)*((tmp_27)*(tmp_27)*(tmp_27))*tmp_33*tmp_34*(tmp_28 + 1)*exp(-tmp_28)/NRPyLeakageET_harm_alpha_fs;
  ...
  const CCTK_REAL tmp_47 = (1.0/64.0)*((NRPyLeakageET_harm_hc3)*(NRPyLeakageET_harm_hc3))*tmp_39*tmp_42*(tmp_19*tmp_26*tmp_33*tmp_41*tmp_46 + tmp_40*tmp_44*tmp_46)/(tmp_26*tmp_34);
