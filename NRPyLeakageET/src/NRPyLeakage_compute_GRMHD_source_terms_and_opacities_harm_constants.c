...
*/
  ...
  const CCTK_REAL tmp_2 = NRPyLeakage_Fermi_Dirac_integrals(4, tmp_1);
  const CCTK_REAL tmp_3 = ((NRPyLeakage_alpha)*(NRPyLeakage_alpha));
  const CCTK_REAL tmp_5 = 8*M_PI*NRPyLeakage_harm_N_A*NRPyLeakage_beta*rho_b_cgs*((3.0/8.0)*tmp_3 + 1.0/8.0)/NRPyLeakage_harm_hc3;
  const CCTK_REAL tmp_6 = ((T)*(T)*(T)*(T)*(T))*tmp_5;
  const CCTK_REAL tmp_7 = NRPyLeakage_Fermi_Dirac_integrals(5, tmp_1);
  const CCTK_REAL tmp_9 = -muhat*tmp_0 + tmp_1;
  const CCTK_REAL tmp_10 = NRPyLeakage_enable_beta_nue*Y_pn/(exp(tmp_9 - tmp_7/tmp_2) + 1);
  const CCTK_REAL tmp_11 = NRPyLeakage_Brems_zeta*NRPyLeakage_enable_brems_nui_anui*rho_b_cgs*(((X_n)*(X_n)) + (28.0/3.0)*X_n*X_p + ((X_p)*(X_p)));
  const CCTK_REAL tmp_13 = NRPyLeakage_Fermi_Dirac_integrals(3, -tmp_1);
  const CCTK_REAL tmp_14 = ((M_PI)*(M_PI));
  const CCTK_REAL tmp_15 = NRPyLeakage_Fermi_Dirac_integrals(3, tmp_1);
  const CCTK_REAL tmp_17 = NRPyLeakage_Fermi_Dirac_integrals(4, -tmp_1);
  const CCTK_REAL tmp_18 = -1.0/2.0*tmp_2/tmp_15 - 1.0/2.0*tmp_17/tmp_13;
  const CCTK_REAL tmp_19 = muhat*tmp_0 - tmp_1;
  const CCTK_REAL tmp_20 = NRPyLeakage_C1pC2_nue_anue*NRPyLeakage_enable_pair_nue_anue/((exp(tmp_18 + tmp_19) + 1)*(exp(tmp_18 + tmp_9) + 1));
  const CCTK_REAL tmp_21 = (1.0/((NRPyLeakage_harm_hc3)*(NRPyLeakage_harm_hc3)));
  const CCTK_REAL tmp_22 = NRPyLeakage_beta*pow(T, 8)*tmp_21;
  const CCTK_REAL tmp_24 = (1.0/3.0)*tmp_14 + ((mu_e)*(mu_e))/((T)*(T));
  const CCTK_REAL tmp_25 = NRPyLeakage_gamma_0*sqrt(tmp_24);
  const CCTK_REAL tmp_27 = ((NRPyLeakage_gamma_0)*(NRPyLeakage_gamma_0))*tmp_24/(tmp_25 + 1);
  const CCTK_REAL tmp_28 = -1.0/2.0*tmp_27 - 1;
  const CCTK_REAL tmp_29 = ((M_PI)*(M_PI)*(M_PI))*pow(NRPyLeakage_gamma_0, 6)*((tmp_24)*(tmp_24)*(tmp_24))*(tmp_25 + 1)*exp(-tmp_25)/NRPyLeakage_harm_alpha_fs;
  const CCTK_REAL tmp_30 = ((NRPyLeakage_C_V)*(NRPyLeakage_C_V))*NRPyLeakage_enable_plasmon_nue_anue*tmp_29/((exp(tmp_19 + tmp_28) + 1)*(exp(tmp_28 + tmp_9) + 1));
  const CCTK_REAL tmp_31 = NRPyLeakage_Brems_C1*pow(T, 4.5)*tmp_11 + (16.0/9.0)*tmp_13*tmp_14*tmp_15*tmp_20*tmp_22 + (1.0/3.0)*tmp_22*tmp_30;
...
tmp_34 = (1 - Y_e)*((5.0/24.0)*tmp_3 + 1.0/24.0)/((2.0/3.0)*MAX(mu_n*tmp_0, 0) + 1);
  const CCTK_REAL tmp_35 = NRPyLeakage_harm_N_A*NRPyLeakage_sigma_0*((T)*(T))*rho_b_cgs/((NRPyLeakage_harm_m_e_c2)*(NRPyLeakage_harm_m_e_c2));
  const CCTK_REAL tmp_36 = (1.0/(NRPyLeakage_Fermi_Dirac_integrals(2, tmp_9)));
  const CCTK_REAL tmp_37 = NRPyLeakage_Fermi_Dirac_integrals(4, tmp_9);
  const CCTK_REAL tmp_39 = tmp_35*tmp_36*tmp_37;
  const CCTK_REAL tmp_40 = (1.0/6.0)*((NRPyLeakage_C_V - 1)*(NRPyLeakage_C_V - 1));
  const CCTK_REAL // Not supported in C:
// MAX
tmp_41 = Y_e*((5.0/24.0)*tmp_3 + tmp_40)/((2.0/3.0)*MAX(mu_p*tmp_0, 0) + 1);
  ...
  const CCTK_REAL tmp_44 = NRPyLeakage_Fermi_Dirac_integrals(5, tmp_9);
  ...
  const CCTK_REAL tmp_47 = (3.0/2.0)*NRPyLeakage_harm_hc3/(M_PI*NRPyLeakage_harm_units_geom_to_cgs_L);
  const CCTK_REAL tmp_48 = tmp_47/((T)*(T)*(T));
  const CCTK_REAL tmp_49 = NRPyLeakage_Fermi_Dirac_integrals(5, -tmp_1);
  const CCTK_REAL tmp_50 = NRPyLeakage_enable_beta_anue*Y_np/(exp(tmp_19 - tmp_49/tmp_17) + 1);
  const CCTK_REAL tmp_51 = tmp_17*tmp_50*tmp_6 + tmp_31;
  const CCTK_REAL tmp_52 = (1.0/(NRPyLeakage_Fermi_Dirac_integrals(2, tmp_19)));
  const CCTK_REAL tmp_53 = NRPyLeakage_Fermi_Dirac_integrals(4, tmp_19);
  const CCTK_REAL tmp_54 = tmp_35*tmp_52*tmp_53;
  const CCTK_REAL tmp_55 = NRPyLeakage_Fermi_Dirac_integrals(5, tmp_19);
  ...
  const CCTK_REAL tmp_58 = NRPyLeakage_Brems_C2*pow(T, 5.5)*tmp_11;
  const CCTK_REAL tmp_59 = pow(T, 9)*tmp_21;
  const CCTK_REAL tmp_61 = (1.0/36.0)*NRPyLeakage_beta*(32*tmp_13*tmp_14*tmp_2*tmp_59 + 32*tmp_14*tmp_15*tmp_17*tmp_59);
  const CCTK_REAL tmp_62 = NRPyLeakage_beta*tmp_59*(tmp_27 + 2);
  const CCTK_REAL tmp_63 = (1.0/(NRPyLeakage_Fermi_Dirac_integrals(3, 0)));
  const CCTK_REAL tmp_64 = tmp_63*NRPyLeakage_Fermi_Dirac_integrals(5, 0);
  ...
  const CCTK_REAL tmp_72 = (1.0/(NRPyLeakage_Fermi_Dirac_integrals(3, tmp_9)));
  ...
  const CCTK_REAL tmp_77 = (1.0/(NRPyLeakage_Fermi_Dirac_integrals(3, tmp_19)));
  const CCTK_REAL tmp_79 = tmp_35*tmp_55*tmp_77;
  const CCTK_REAL tmp_80 = tmp_34*tmp_79 + tmp_42*tmp_55*tmp_77 + tmp_56*tmp_79;
  const CCTK_REAL tmp_81 = NRPyLeakage_Fermi_Dirac_integrals(4, 0)/NRPyLeakage_Fermi_Dirac_integrals(2, 0);
  ...
  *Q_source = NRPyLeakage_harm_units_cgs_to_geom_Q*(-tmp_69/(((tau_nue[1])*(tau_nue[1]))*tmp_69*tmp_70*tmp_72/tmp_75 + 1) - tmp_76/(((tau_anue[1])*(tau_anue[1]))*tmp_70*tmp_76*tmp_77/tmp_80 + 1) - 4*(NRPyLeakage_C1pC2_nux_anux*NRPyLeakage_enable_pair_nux_anux*tmp_61/((exp(tmp_18) + 1)*(exp(tmp_18) + 1)) + NRPyLeakage_enable_plasmon_nux_anux*tmp_29*tmp_40*tmp_62/((exp(tmp_28) + 1)*(exp(tmp_28) + 1)) + tmp_58)/(((tau_nux[1])*(tau_nux[1]))*tmp_63*tmp_69*tmp_70/tmp_66 + 1));
  kappa_nue[0] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_46;
  kappa_nue[1] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_75;
  kappa_anue[0] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_57;
