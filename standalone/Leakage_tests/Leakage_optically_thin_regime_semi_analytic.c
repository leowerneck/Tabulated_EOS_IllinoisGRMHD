...
inline
void rk4_step_ode( const NRPyEOS_params *restrict eos_params,
                   const REAL dt,
                   const REAL rho_b,
                   REAL *restrict gfs ) {
...
void Leakage_optically_thin_regime_semi_analytic( const NRPyEOS_params *restrict eos_params,
  ...
  for(int n=0;n<n_steps;n++) {
    rk4_step_ode(eos_params,dt,initial_rho_b,gfs);
    t+= dt;
    REAL T = 1.0;
