...
#define READ_BCAST_EOSTABLE_HDF5(NAME,OFF,DIMS)                          \
  ...
  if (!(alltables_temp = (double*)malloc(eos_params->nrho * eos_params->ntemp * eos_params->nye * NTABLES * sizeof(double)))) {
  ...
  hsize_t table_dims[2] = {NTABLES, (hsize_t)eos_params->nrho * eos_params->ntemp * eos_params->nye};
  ...
  if (!(eos_params->alltables = (double*)malloc(eos_params->nrho * eos_params->ntemp * eos_params->nye * NTABLES 
				    * sizeof(double)))) {
    fprintf(stderr,"(NRPyEOS) Cannot allocate memory for EOS table");
  }
  for(int iv = 0;iv<NTABLES;iv++) 
	...
	for(int i = 0; i<eos_params->nrho; i++) {
	  ...
	  int indnew = iv + NTABLES*(i + eos_params->nrho*(j + eos_params->ntemp*k));
