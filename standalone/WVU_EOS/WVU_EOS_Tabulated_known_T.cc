...
void WVU_EOS_P_and_eps_from_rho_Ye_T_impl( const REAL rho,
  ...
  // Error handling
  if( report.error ) {
    fprintf(stderr,"(WVU_EOS) Inside WVU_EOS_P_and_eps_from_rho_Ye_T. Error message: %s (key = %d)",report.message.c_str(),report.error_key);
...
void WVU_EOS_P_eps_and_S_from_rho_Ye_T_impl( const REAL rho,
  ...
  const int n = 3;
  // Table variables keys
  const int keys[n] = {WVU_EOS::press_key,WVU_EOS::eps_key,WVU_EOS::entropy_key};
  // Declare error variable
  WVU_EOS::eos_error_report report;
  // Set output variable array
  REAL outvars[n];
  ...
  // Error handling
  if( report.error ) {
    fprintf(stderr,"(WVU_EOS) Inside WVU_EOS_P_eps_and_S_from_rho_Ye_T. Error message: %s (key = %d)",report.message.c_str(),report.error_key);
    // May want to terminate depending on the error. We'll just warn for now.
  }
...
void WVU_EOS_P_eps_S_and_cs2_from_rho_Ye_T_impl( const REAL rho,
  ...
  const int n = 4;
  // Table variables keys
  const int keys[n] = {WVU_EOS::press_key,WVU_EOS::eps_key,WVU_EOS::entropy_key,WVU_EOS::cs2_key};
  // Declare error variable
  WVU_EOS::eos_error_report report;
  // Set output variable array
  REAL outvars[n];
...
void WVU_EOS_P_eps_and_depsdT_from_rho_Ye_T_impl( const REAL rho,
  ...
  const int n = 3;
  // Table variables keys
  const int keys[n] = {WVU_EOS::press_key,WVU_EOS::eps_key,WVU_EOS::depsdT_key};
  // Declare error variable
  WVU_EOS::eos_error_report report;
  // Set output variable array
  REAL outvars[n];
...
void WVU_EOS_P_eps_dPdrho_dPdT_depsdrho_and_depsdT_from_rho_Ye_T_impl( const REAL rho,
  ...
  const int keys[n] = {WVU_EOS::press_key,WVU_EOS::eps_key,WVU_EOS::depsdT_key,WVU_EOS::dPdrho_key,WVU_EOS::dPdeps_key};
  // Declare error variable
  WVU_EOS::eos_error_report report;
  // Set output variable array
  REAL outvars[n];
