...
#define HDF5_DO_IO(fn_call)                                              \
  {                                                                      \
    ...
    if (_error_code < 0) {	       				         \
      fprintf(stderr,"(WVU_EOS) HDF5 call '%s' returned error code %d",  \
                  #fn_call, _error_code);                                \
    }                                                                    \
  }
...
void WVU_EOS_ReadTable(const char *nuceos_table_name) {
  ...
  printf("(WVU_EOS) *******************************\n");
  printf("(WVU_EOS) Reading nuc_eos table file:\n");
  printf("(WVU_EOS) %s\n",nuceos_table_name);
  printf("(WVU_EOS) *******************************\n");
  ...
  // {
  //   fprintf(stderr,"(WVU_EOS) Requested IO process %d out of range. Reverting to process 0.\n", my_reader_process);
  //   my_reader_process = 0;
  // }
  ...
  //   fprintf(stderr,"(WVU_EOS) Could not read nuceos_table_name %s \n",
  //              nuceos_table_name);
  // }
...
#define READ_BCAST_EOSTABLE_HDF5(NAME,OFF,DIMS)                          \
  ...
  if (!(alltables_temp = (double*)malloc(nrho * ntemp * nye * NTABLES * sizeof(double)))) {
    fprintf(stderr,"(WVU_EOS) Cannot allocate memory for EOS table");
  }
  ...
  if (!(epstable = (double*)malloc(nrho * ntemp * nye  
				    * sizeof(double)))) {
    fprintf(stderr,"(WVU_EOS) Cannot allocate memory for eps table\n");
  }
