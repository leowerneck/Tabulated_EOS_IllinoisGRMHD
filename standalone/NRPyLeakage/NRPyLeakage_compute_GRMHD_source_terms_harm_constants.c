...
*/
  ...
  const REAL tmp_38 = NRPyLeakage_beta*(32*tmp_11*tmp_12*tmp_2*tmp_36 + 32*tmp_12*tmp_13*tmp_15*tmp_36);
  const REAL tmp_39 = (1.0/6.0)*NRPyLeakage_beta*tmp_36*(tmp_24 + 2);
  const REAL tmp_40 = pow(T, 6)*tmp_3;
  const REAL tmp_41 = (1.0/36.0)*tmp_18*tmp_38 + tmp_27*tmp_39 + tmp_35;
  ...
  *Q_source = NRPyLeakage_harm_units_cgs_to_geom_Q*(-tmp_42/(t_diff_1_nue*tmp_42*tmp_43/NRPyLeakage_Fermi_Dirac_integrals(3, tmp_7) + 1) - tmp_45/(t_diff_1_anue*tmp_43*tmp_45/NRPyLeakage_Fermi_Dirac_integrals(3, tmp_17) + 1) - 4*((1.0/9.0)*NRPyLeakage_C1pC2_nux_anux*NRPyLeakage_enable_pair_nux_anux*tmp_38/((exp(tmp_16) + 1)*(exp(tmp_16) + 1)) + NRPyLeakage_enable_plasmon_nux_anux*tmp_26*tmp_39*((NRPyLeakage_C_V - 1)*(NRPyLeakage_C_V - 1))/((exp(tmp_25) + 1)*(exp(tmp_25) + 1)) + tmp_35)/(t_diff_1_nux*tmp_42*tmp_43/NRPyLeakage_Fermi_Dirac_integrals(3, 0) + 1));
}
