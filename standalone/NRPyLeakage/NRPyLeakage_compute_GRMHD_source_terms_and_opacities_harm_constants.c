...
void NRPyLeakage_compute_GRMHD_source_terms_and_opacities_harm_constants(const NRPyEOS_params *restrict eos_params,
  ...
  REAL mu_e, mu_p, mu_n, muhat, X_p, X_n, X_h;
  NRPyEOS_mue_mup_mun_muhat_Xn_Xp_and_Xh_from_rho_Ye_T(eos_params, rho_b, Y_e, T,
                                                       &mu_e, &mu_p, &mu_n, &muhat, &X_p, &X_n, &X_h);
  ...
  REAL Y_pn, Y_np;
  if( X_h < 0.5 ) {
    Y_np = MAX( (2*Y_e-1)/(exp_metahat-1), 0.0 );
    Y_pn = MAX( exp_metahat*Y_np, 0.0 );
  }
  else {
    Y_np = X_n;
    Y_pn = X_p;
  ...
  const REAL tmp_7 = M_PI/NRPyLeakage_harm_hc3;
  const REAL tmp_9 = 8*NRPyLeakage_harm_N_A*NRPyLeakage_beta*((T)*(T)*(T)*(T)*(T))*rho_b_cgs*tmp_7*((3.0/8.0)*tmp_6 + 1.0/8.0);
  ...
  const REAL tmp_23 = (1.0/((NRPyLeakage_harm_hc3)*(NRPyLeakage_harm_hc3)));
  ...
  const REAL tmp_36 = (1.0/3.0)*((M_PI)*(M_PI)*(M_PI))*NRPyLeakage_beta*pow(NRPyLeakage_gamma_0, 6)*tmp_23*tmp_25*((tmp_31)*(tmp_31)*(tmp_31))*(tmp_32 + 1)*exp(-tmp_32)/NRPyLeakage_harm_alpha_fs;
  ...
  const REAL tmp_44 = (1.0/64.0)*((NRPyLeakage_harm_hc3)*(NRPyLeakage_harm_hc3))*tmp_17*tmp_19*(tmp_12*tmp_24*tmp_43 + tmp_18*tmp_2*tmp_21*tmp_23*tmp_43)/(tmp_21*tmp_25);
  ...
  const REAL tmp_55 = NRPyLeakage_harm_N_A*NRPyLeakage_sigma_0*((T)*(T))*rho_b_cgs/((NRPyLeakage_harm_m_e_c2)*(NRPyLeakage_harm_m_e_c2));
  ...
  const REAL tmp_69 = 6/NRPyLeakage_harm_units_geom_to_cgs_L;
  ...
  *R_source = NRPyLeakage_amu*NRPyLeakage_harm_units_cgs_to_geom_R*(-tmp_70 + tmp_83);
  *Q_source = NRPyLeakage_harm_units_cgs_to_geom_Q*(-tmp_104 - 4*tmp_111 - tmp_95);
  kappa_nue[0] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_66;
  kappa_nue[1] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_92;
  kappa_anue[0] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_82;
  kappa_anue[1] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_103;
  kappa_nux[0] = NRPyLeakage_harm_units_geom_to_cgs_L*(tmp_113 + tmp_114);
  kappa_nux[1] = NRPyLeakage_harm_units_geom_to_cgs_L*(tmp_108 + tmp_109);
