...
*/
  ...
  const REAL tmp_33 = ((M_PI)*(M_PI)*(M_PI))*pow(NRPyLeakage_gamma_0, 6)*((tmp_30)*(tmp_30)*(tmp_30))*(tmp_31 + 1)*exp(-tmp_31)/NRPyLeakage_harm_alpha_fs;
  ...
  const REAL tmp_60 = NRPyLeakage_harm_N_A*NRPyLeakage_sigma_0*((T)*(T))*rho_b_cgs/((NRPyLeakage_harm_m_e_c2)*(NRPyLeakage_harm_m_e_c2));
...
tmp_65 = Y_e*((5.0/24.0)*tmp_3 + (1.0/6.0)*tmp_39)/((2.0/3.0)*MAX(mu_p*tmp_0, 0) + 1);
  ...
  const REAL tmp_72 = (3.0/2.0)*NRPyLeakage_harm_hc3/(M_PI*NRPyLeakage_harm_units_geom_to_cgs_L);
  ...
  *R_source = NRPyLeakage_amu*NRPyLeakage_harm_units_cgs_to_geom_R*(-tmp_74 + tmp_87);
  *Q_source = NRPyLeakage_harm_units_cgs_to_geom_Q*(-tmp_108 - 4*tmp_115 - tmp_99);
  kappa_nue[0] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_71;
  kappa_nue[1] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_96;
  kappa_anue[0] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_86;
  kappa_anue[1] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_107;
  kappa_nux[0] = NRPyLeakage_harm_units_geom_to_cgs_L*(tmp_116*tmp_81 + tmp_117);
  kappa_nux[1] = NRPyLeakage_harm_units_geom_to_cgs_L*tmp_114;
