...
*/
  ...
  const REAL tmp_22 = (1.0/((NRPyLeakage_hc3)*(NRPyLeakage_hc3)));
  ...
  const REAL tmp_30 = ((M_PI)*(M_PI)*(M_PI))*pow(NRPyLeakage_gamma_0, 6)*((tmp_27)*(tmp_27)*(tmp_27))*(tmp_28 + 1)*exp(-tmp_28)/NRPyLeakage_alpha_fs;
  ...
  const REAL tmp_53 = (1.0/4.0)*NRPyLeakage_hc3/M_PI;
  ...
  Q_eff_anue = tmp_64;
  Q_eff_nux = tmp_65;
  *R_source = NRPyLeakage_amu*NRPyLeakage_units_cgs_to_geom_R*(-tmp_55 + tmp_57);
  *Q_source = NRPyLeakage_units_cgs_to_geom_Q*(-tmp_62 - tmp_64 - 4*tmp_65);
