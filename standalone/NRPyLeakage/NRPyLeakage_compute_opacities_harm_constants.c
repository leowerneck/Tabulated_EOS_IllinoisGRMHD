...
void NRPyLeakage_compute_opacities_harm_constants(const NRPyEOS_params *restrict eos_params,
  ...
  const REAL tmp_5 = NRPyLeakage_harm_N_A*NRPyLeakage_sigma_0*((T)*(T))*rho_b_cgs/((NRPyLeakage_harm_m_e_c2)*(NRPyLeakage_harm_m_e_c2));
  ...
  fprintf(stderr,"(NRPyLeakage) eta_hat          = %.15e\n",muhat/T);
  fprintf(stderr,"(NRPyLeakage) expmetahat       = %.15e\n",exp_metahat);
  fprintf(stderr,"(NRPyLeakage) Y_nn             = %.15e\n",(1 - Y_e)/((2.0/3.0)*MAX(mu_n*tmp_0, 0) + 1));
  fprintf(stderr,"(NRPyLeakage) Y_pp             = %.15e\n",     Y_e /((2.0/3.0)*MAX(mu_p*tmp_0, 0) + 1));
  fprintf(stderr,"(NRPyLeakage) Y_pn             = %.15e\n",Y_pn);
  fprintf(stderr,"(NRPyLeakage) Y_np             = %.15e\n",Y_np);
