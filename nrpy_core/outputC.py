...
outC_function_tuple = namedtuple('outC_function_dict_tuple','function includes prefunc desc c_type name params preloop body loopopts postloop enableCparameters rel_path_to_Cparams uses_rfm use_cuda')
...
def Cfunction(includes=None, prefunc="", desc="", c_type="void", name=None, params=None, preloop="", body=None,
              loopopts="", postloop="", enableCparameters=True, rel_path_to_Cparams=os.path.join("./"), uses_rfm=False, use_cuda=False):
...
def add_to_Cfunction_dict(includes=None, prefunc="", desc="", c_type="void", name=None, params=None,
                          ...
                          rel_path_to_Cparams=os.path.join("./"), uses_rfm=False, use_cuda=False):
    outC_function_outdir_dict[name] = path_from_rootsrcdir_to_this_Cfunc
    Cfunc_prototype, Cfunc = Cfunction(includes, prefunc, desc, c_type, name, params, preloop, body, loopopts, postloop,
                                       enableCparameters, rel_path_to_Cparams, uses_rfm, use_cuda)
    outC_function_prototype_dict[name] = Cfunc_prototype
    outC_function_dict[name] = outC_function_tuple(Cfunc, includes, prefunc, desc, c_type, name, params, preloop, body, loopopts, postloop,
                                                   enableCparameters, rel_path_to_Cparams, uses_rfm, use_cuda)


def outCfunction(outfile="", includes=None, prefunc="", desc="",
                 c_type="void", name=None, params=None, preloop="", body=None, loopopts="", postloop="",
                 enableCparameters=True, rel_path_to_Cparams=os.path.join("./"), uses_rfm=False, use_cuda=False):
    _ignoreprototype,Cfunc = Cfunction(includes, prefunc, desc, c_type, name, params, preloop, body,
                                       loopopts, postloop, enableCparameters, rel_path_to_Cparams, uses_rfm, use_cuda)
...
def construct_Makefile_from_outC_function_dict(Ccodesrootdir, exec_name, uses_free_parameters_h=False,
    ...
    use_cuda = False
    for key in outC_function_dict:
        item = outC_function_dict[key].function
        if outC_function_dict[key].use_cuda == False:
            ext  = ".c"
        else:
            ext = ".cu"
            use_cuda = True
        ...
        if "grid" in key.split("_")[-1] and not "grids" in key:
            ...
            with open(add_to_Makefile(Ccodesrootdir, os.path.join(subdir, key+ext)), "w") as file:
                file.write(item)
            ...
            with open(add_to_Makefile(Ccodesrootdir, os.path.join(key+ext)), "w") as file:
                file.write(item)
    ...
    for c_file in Makefile_list_of_files:
        if ".cu" in c_file:
            object_file = c_file.replace(".cu", ".o")
        else:
        ...
        if ".cu" in c_file:
            compile_list.append("\tnvcc -c " + c_file + " -o " + object_file)
        else:
            compile_list.append("\t$(CC) $(CFLAGS) -c " + c_file + " -o " + object_file)

    linked_libraries = " -lm"
    if use_cuda:
        linked_libraries += " -lcuda"
