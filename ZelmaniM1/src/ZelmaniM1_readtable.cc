...
namespace ZelmaniM1 {
  ...
  void zm1_readtable(CCTK_ARGUMENTS) {
    ...
    if (false) {
      ...
      struct stat fstat;
      if (stat(nulib_table_name,&fstat)==-1) {
        CCTK_VWarn(CCTK_WARN_ABORT, __LINE__, __FILE__, CCTK_THORNSTRING,
                   "Could not read nulib_table_name'%s'",
                   nulib_table_name);
      }
...
#define READ_SHIT_HDF5(NAME,VAR,TYPE,NELMS)                                     \
    ...
    absorb_temp = (double*) malloc(sizeof(double)*ng*ns*nrho*ntemp*nye);
    emis_temp = (double*) malloc(sizeof(double)*ng*ns*nrho*ntemp*nye);
    scat_temp = (double*) malloc(sizeof(double)*ng*ns*nrho*ntemp*nye);
    hsize_t table_dims[5] = {ng,ns,ntemp,nye,nrho};
    
    READ_SHIT_HDF5("absorption_opacity",absorb_temp,H5T_NATIVE_DOUBLE,table_dims);
    READ_SHIT_HDF5("scattering_opacity",scat_temp,H5T_NATIVE_DOUBLE,table_dims);
    READ_SHIT_HDF5("emissivities",emis_temp,H5T_NATIVE_DOUBLE,table_dims);
