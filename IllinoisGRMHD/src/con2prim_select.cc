...
int con2prim_select(
  ...
  switch(c2p_key) {
    ...
    case igm_Palenzuela1D:
      return con2prim_Palenzuela1D(eos, adm_quantities, cons, prim, stats);

    // Palenzuela 1D routine (see https://arxiv.org/pdf/1712.07538.pdf)
    case igm_Palenzuela1D_entropy:
      return con2prim_Palenzuela1D_entropy(eos, adm_quantities, cons, prim, stats);
    ...
    // Newman 1D routine (see https://escholarship.org/content/qt0s53f84b/qt0s53f84b.pdf)
    case igm_Newman1D_entropy:
      return con2prim_Newman1D_entropy(eos, adm_quantities, cons, prim, stats);

    default:
      CCTK_VERROR("Unknown c2p key in con2prim_select (%d). ABORTING!", c2p_key);
  }
}
