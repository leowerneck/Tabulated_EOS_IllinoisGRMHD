...
inline int check_depsdT_condition( const igm_eos_parameters eos,
  ...
  int con2prim_key = igm_None;
  if( eos.evolve_entropy && (depsdT < eos.depsdT_threshold) ) {
    con2prim_key = igm_Palenzuela1D_entropy;
  }
  else {
    con2prim_key = igm_Palenzuela1D;
  }
...
int con2prim_Palenzuela1D( const igm_eos_parameters eos,
  ...
  if( S_squared > 0.9999 * S_squared_max ) {
    ...
    if( simple_rel_err(S_squared,0.9999*S_squared_max) > 1e-12 ) CCTK_VERROR("Incompatible values of S_squared after rescaling: %.15e %.15e\n",S_squared,0.9999*S_squared_max);
  }
  ...
  if( (stats.c2p_failed == true) && (stats.which_routine == igm_Palenzuela1D_entropy) ) {
...
void palenzuela( const igm_eos_parameters eos,
  ...
  if( stats.which_routine == igm_None ) {
    ...
    if( (xlow_entropy_key == igm_Palenzuela1D_entropy) ||
        (xup_entropy_key  == igm_Palenzuela1D_entropy) ) {
