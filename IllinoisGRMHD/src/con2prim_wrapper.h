int con2prim(
      ...
      const CCTK_REAL *restrict METRIC_PHYS,
      const CCTK_REAL *restrict METRIC_LAP_PSI4,
      const CCTK_REAL g4dn[NDIM][NDIM],
      const CCTK_REAL g4up[NDIM][NDIM],
      CCTK_REAL *restrict CONSERVS,
  ...
  */
  ...
  // FIXME: Only works if poisoning is turned on. Otherwise will access unknown memory. This trick alone speeds up the
  // whole code (Cowling) by 2%. int startguess=0; if(robust_isnan(PRIMS[VX])) startguess=1;
  int startguess = 1;
  ...
  for(int which_guess = startguess; which_guess < 3; which_guess++) {
    ...
    set_prim_from_PRIMS_and_CONSERVS(
          eos, eos.c2p_routine, which_guess, METRIC, METRIC_LAP_PSI4, PRIMS, CONSERVS, cons, prim);
    ...
    if((check != 0) && (eos.c2p_backup[0] != igm_None)) {
      ...
      set_prim_from_PRIMS_and_CONSERVS(
            eos, eos.c2p_backup[0], which_guess, METRIC, METRIC_LAP_PSI4, PRIMS, CONSERVS, cons, prim);
      // Backup routine #1
      ...
      if((check != 0) && (eos.c2p_backup[1] != igm_None)) {
        ...
        set_prim_from_PRIMS_and_CONSERVS(
              eos, eos.c2p_backup[1], which_guess, METRIC, METRIC_LAP_PSI4, PRIMS, CONSERVS, cons, prim);
        // Backup routine #2
        ...
        if((check != 0) && (eos.c2p_backup[2] != igm_None)) {
          ...
          set_prim_from_PRIMS_and_CONSERVS(
                eos, eos.c2p_backup[2], which_guess, METRIC, METRIC_LAP_PSI4, PRIMS, CONSERVS, cons, prim);
          // Backup routine #3
    ...
    if(eos.is_Hybrid) {
      ...
      if(check != 0) {
        ...
        if(check == 1) {
          ...
          CCTK_VInfo(
                CCTK_THORNSTRING,
                "i,j,k = %d %d %d, stats.failure_checker = %d x,y,z = %e %e %e , index=%d st_i = %e %e %e, rhostar = "
                "%e, Bi = %e %e %e, gij = %e %e %e %e %e %e, Psi6 = %e",
                i, j, k, stats.failure_checker, X[index], Y[index], Z[index], index, mhd_st_x_orig, mhd_st_y_orig,
                mhd_st_z_orig, rho_star_orig, PRIMS[BX_CENTER], PRIMS[BY_CENTER], PRIMS[BZ_CENTER], METRIC_PHYS[GXX],
                METRIC_PHYS[GXY], METRIC_PHYS[GXZ], METRIC_PHYS[GYY], METRIC_PHYS[GYZ], METRIC_PHYS[GZZ],
    ...
    if(check == 0) {
      ...
      // Check for NAN!
      if(robust_isnan(
               prim[RHO] * prim[TEMP] * prim[YE] * prim[PRESS] * prim[EPS] * prim[ENT] * utx_new * uty_new * utz_new
               * u0L)) {
        CCTK_VINFO("***********************************************************");
        CCTK_VINFO("NAN found in function %s (file: %s)", __func__, __FILE__);
        CCTK_VINFO("Input IllinoisGRMHD conserved variables:");
        CCTK_VINFO(
              "rho_*, Ye_*, ~tau, ~S_{i}: %e %e %e %e %e %e", CONSERVS[RHOSTAR], CONSERVS[YESTAR], CONSERVS[TAUENERGY],
              CONSERVS[STILDEX], CONSERVS[STILDEY], CONSERVS[STILDEZ]);
        CCTK_VINFO("Input con2prim conserved variables:");
        CCTK_VINFO(
              "D, DYe, tau, S_{i}: %e %e %e %e %e %e", cons[RHO], cons[YE], cons[TAU], cons[S1_cov], cons[S2_cov],
              cons[S3_cov]);
