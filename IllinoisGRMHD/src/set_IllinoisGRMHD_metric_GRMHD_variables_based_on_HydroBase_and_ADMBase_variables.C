...
#pragma omp parallel for
  for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) for(int i=0;i<cctk_lsh[0];i++) {
       ...
       //     const CCTK_REAL prsL     = press[index];
       ...
       //     if( eos.evolve_entropy ) {
       //       entropy[index] = entropy[index] = entropyL;
       //     }
       // }
        ...
        if( eos.is_Hybrid ) {
           ...
           */
          /* Compute P_cold */
          const int ppidx         = find_polytropic_K_and_Gamma_index(eos, rho[index]);
          const double K_poly     = eos.K_ppoly_tab[ppidx];
          const double Gamma_poly = eos.Gamma_ppoly_tab[ppidx];
          const double P_cold     = K_poly*pow(rho[index],Gamma_poly);

          /* Compare P and P_cold */
          double P_rel_error = fabs(press[index] - P_cold)/press[index];
          if( rho[index] > rho_b_atm && P_rel_error > 1e-2 ) {
            const double Gamma_poly_local = log(press[index]/K_poly) / log(rho[index]);
            ...
            CCTK_VWarn(CCTK_WARN_ALERT, __LINE__, __FILE__, CCTK_THORNSTRING,
                       ...
                       rho[index], rho_b_atm, press[index],P_cold,P_rel_error);
          }
        }
