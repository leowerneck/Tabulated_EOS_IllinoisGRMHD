...
static double zbrent(
      double (*func)(const igm_eos_parameters, double, double *restrict, double *restrict, output_stats &),
      const igm_eos_parameters eos,
...
int con2prim_Palenzuela1D_entropy(
  ...
  if(S_squared > 0.9999 * S_squared_max) {
    ...
    // Rescale S_{i}
    for(int i = 0; i < 3; i++) {
      SD[i] *= rescale_factor_must_be_less_than_one;
    }
    ...
    if(simple_rel_err(S_squared, 0.9999 * S_squared_max) > 1e-12) {
      CCTK_VERROR(
            "Incompatible values of S_squared after rescaling: %.15e %.15e\n", S_squared, 0.9999 * S_squared_max);
    }
  }
...
static void calc_prim(
  ...
  double ent   = 0.0;
  if(eos.evolve_T) {
    if(stats.which_routine == igm_Palenzuela1D) {
...
static double zbrent(
      double (*func)(const igm_eos_parameters, double, double *, double *restrict, output_stats &),
      const igm_eos_parameters eos,
      double *restrict param,
      double *temp_guess,
      double x1,
  ...
  while(keep_iterating) {
    ...
    if(((fabs(maxerror) <= tol1) && (doing_extra == 0)) || (i_extra > EXTRA_NEWT_ITER) || (iter >= (ITMAX))) {
      keep_iterating = 0;
    }
