...
// These BSSN-based variables are not evolved, and so are not defined anywhere
// that the grid has moved. Here we convert ADM variables (from ADMBase) to the
// BSSN-based variables expected by this routine.
IllinoisGRMHD_convert_ADM_to_BSSN__enforce_detgtij_eq_1__and_compute_gtupij(
    cctkGH, cctk_lsh, gxx, gxy, gxz, gyy, gyz, gzz, alp, gtxx, gtxy, gtxz, gtyy,
    gtyz, gtzz, gtupxx, gtupxy, gtupxz, gtupyy, gtupyz, gtupzz, phi_bssn,
    psi_bssn, lapm1);
...
// FIXME: Replace this timing benchmark with something more meaningful, like the
// avg # of Newton-Raphson iterations per gridpoint!
/*
...
int failures = 0, font_fixes = 0, vel_limited_ptcount = 0, atm_resets = 0,
    rho_tilde_fix_applied = 0;
int pointcount = 0;
...
#pragma omp parallel for reduction(                                            \
        + : failures, vel_limited_ptcount, font_fixes, pointcount,             \
            failures_inhoriz, pointcount_inhoriz, error_int_numer,             \
            error_int_denom, rho_tilde_fix_applied, atm_resets, backup1,       \
            backup2, backup3, num_of_conservative_averagings_needed,           \
            nan_found) schedule(static)
      ...
      for (int i = imin; i < imax; i++) {
        ...
        if ((loop_count == 0) || (c2p_fail_flag != 0)) {
          ...
          CCTK_REAL CONSERVS[NUM_CONSERVS],
              CONSERVS_avg_neighbors[NUM_CONSERVS];
          CONSERVS[RHOSTAR] = rho_tilde[index];
          CONSERVS[STILDEX] = S_x_tilde[index];
          CONSERVS[STILDEY] = S_y_tilde[index];
          CONSERVS[STILDEZ] = S_z_tilde[index];
          CONSERVS[TAUENERGY] = tau_tilde[index];
          CONSERVS[YESTAR] = Y_e_tilde[index];
          CONSERVS[ENTSTAR] = ent_tilde[index];
          ...
          if (c2p_fail_flag != 0) {
            ...
            int __attribute__((unused)) neighbors =
                con2prim_average_neighbor_conservatives(
                    cctkGH, i, j, k, index, cctk_lsh, con2prim_failed_flag,
                    rho_tilde, S_x_tilde, S_y_tilde, S_z_tilde, tau_tilde,
                    Y_e_tilde, ent_tilde, CONSERVS_avg_neighbors);
            if (neighbors > 0) {
              ...
              for (int which_con = 0; which_con < NUM_CONSERVS; which_con++)
                CONSERVS[which_con] =
                    CONSERVS[which_con] * one_minus_C_weight +
                    CONSERVS_avg_neighbors[which_con] * C_weight;
              ...
              // Probably should terminate in this case, but let us reset to ATM
              // for now. Simplest way is to set rhostar to a negative value.
          ...
          CCTK_REAL shift_xL = METRIC_PHYS[GXX] * METRIC[SHIFTX] +
                               METRIC_PHYS[GXY] * METRIC[SHIFTY] +
                               METRIC_PHYS[GXZ] * METRIC[SHIFTZ];
          CCTK_REAL shift_yL = METRIC_PHYS[GXY] * METRIC[SHIFTX] +
                               METRIC_PHYS[GYY] * METRIC[SHIFTY] +
                               METRIC_PHYS[GYZ] * METRIC[SHIFTZ];
          CCTK_REAL shift_zL = METRIC_PHYS[GXZ] * METRIC[SHIFTX] +
                               METRIC_PHYS[GYZ] * METRIC[SHIFTY] +
                               METRIC_PHYS[GZZ] * METRIC[SHIFTZ];
          CCTK_REAL beta2L = shift_xL * METRIC[SHIFTX] +
                             shift_yL * METRIC[SHIFTY] +
                             shift_zL * METRIC[SHIFTZ];

          // Compute 4-metric, both g_{\mu \nu} and g^{\mu \nu}.
          // This is for computing T_{\mu \nu} and T^{\mu \nu}. Also the HARM
          // con2prim lowlevel function requires them.
          ...
          g4up[1][1] = METRIC_PHYS[GUPXX] -
                       METRIC[SHIFTX] * METRIC[SHIFTX] * alpha_inv_squared;
          g4up[1][2] = g4up[2][1] = METRIC_PHYS[GUPXY] - METRIC[SHIFTX] *
                                                             METRIC[SHIFTY] *
                                                             alpha_inv_squared;
          g4up[1][3] = g4up[3][1] = METRIC_PHYS[GUPXZ] - METRIC[SHIFTX] *
                                                             METRIC[SHIFTZ] *
                                                             alpha_inv_squared;
          g4up[2][2] = METRIC_PHYS[GUPYY] -
                       METRIC[SHIFTY] * METRIC[SHIFTY] * alpha_inv_squared;
          g4up[2][3] = g4up[3][2] = METRIC_PHYS[GUPYZ] - METRIC[SHIFTY] *
                                                             METRIC[SHIFTZ] *
                                                             alpha_inv_squared;
          g4up[3][3] = METRIC_PHYS[GUPZZ] -
                       METRIC[SHIFTZ] * METRIC[SHIFTZ] * alpha_inv_squared;

          // FIXME: might slow down the code.
          if (robust_isnan(CONSERVS[RHOSTAR] * CONSERVS[STILDEX] *
                           CONSERVS[STILDEY] * CONSERVS[STILDEZ] *
                           CONSERVS[TAUENERGY] * PRIMS[BX_CENTER] *
                           PRIMS[BY_CENTER] * PRIMS[BZ_CENTER])) {
            CCTK_VWARN(CCTK_WARN_ALERT,
                       "NAN FOUND: i,j,k = %d %d %d, x,y,z = %e %e %e , "
                       "index=%d st_i = %e %e %e, rhostar = %e, tau = %e, Bi = "
                       "%e %e %e, gij = %e %e %e %e %e %e, Psi6 = %e",
                       i, j, k, x[index], y[index], z[index], index,
                       CONSERVS[STILDEX], CONSERVS[STILDEY], CONSERVS[STILDEZ],
                       CONSERVS[RHOSTAR], CONSERVS[TAUENERGY], PRIMS[BX_CENTER],
                       PRIMS[BY_CENTER], PRIMS[BZ_CENTER], METRIC_PHYS[GXX],
                       METRIC_PHYS[GXY], METRIC_PHYS[GXZ], METRIC_PHYS[GYY],
                       METRIC_PHYS[GYZ], METRIC_PHYS[GZZ],
                       METRIC_LAP_PSI4[PSI6]);
            nan_found++;
          }

          // Here we use _flux variables as temp storage for original values of
          // conservative variables.. This is used for debugging purposes only.
          rho_flux[index] = CONSERVS[RHOSTAR];
          S_x_flux[index] = CONSERVS[STILDEX];
          S_y_flux[index] = CONSERVS[STILDEY];
          S_z_flux[index] = CONSERVS[STILDEZ];
          tau_flux[index] = CONSERVS[TAUENERGY];
          Y_e_flux[index] = CONSERVS[YESTAR];
          ent_flux[index] = CONSERVS[ENTSTAR];

          CCTK_REAL rho_tilde_orig = CONSERVS[RHOSTAR];
          CCTK_REAL S_x_tilde_orig = CONSERVS[STILDEX];
          CCTK_REAL S_y_tilde_orig = CONSERVS[STILDEY];
          CCTK_REAL S_z_tilde_orig = CONSERVS[STILDEZ];
          CCTK_REAL tau_tilde_orig = CONSERVS[TAUENERGY];
          CCTK_REAL Y_e_tilde_orig = 0.0;
          CCTK_REAL ent_tilde_orig = 0.0;
          if (eos.is_Tabulated) {
            Y_e_tilde_orig = CONSERVS[YESTAR];
          }
          if (eos.evolve_entropy) {
            ent_tilde_orig = CONSERVS[ENTSTAR];
          }
          ...
          if (CONSERVS[RHOSTAR] > 0.0) {
            // Apply the tau floor
            if (eos.is_Hybrid) {
              apply_tau_floor(index, Psi6threshold, PRIMS, METRIC, METRIC_PHYS,
                              METRIC_LAP_PSI4, stats, eos, CONSERVS);
            }

            for (int ii = 0; ii < 3; ii++) {
              check =
                  con2prim(eos, index, i, j, k, x, y, z, METRIC, METRIC_PHYS,
                           METRIC_LAP_PSI4, g4dn, g4up, CONSERVS, PRIMS, stats);
              if (check == 0)
                ii = 4;
              else
                stats.failure_checker += 100000;
            }
          } else {
            stats.failure_checker += 1;
            reset_prims_to_atmosphere(eos, PRIMS);
            rho_tilde_fix_applied++;
          }
            ...
            if (con2prim_failed_flag[index] > 4) {
              ...
              if (eos.is_Hybrid) {
                CCTK_VInfo(CCTK_THORNSTRING,
                           "Couldn't find root from: %e %e %e %e %e, rhob "
                           "approx=%e, rho_b_atm=%e, Bx=%e, By=%e, Bz=%e, "
                           "gij_phys=%e %e %e %e %e %e, alpha=%e",
                           tau_tilde_orig, rho_tilde_orig, S_x_tilde_orig,
                           S_y_tilde_orig, S_z_tilde_orig,
                           rho_tilde_orig / METRIC_LAP_PSI4[PSI6],
                           eos.rho_b_atm, PRIMS[BX_CENTER], PRIMS[BY_CENTER],
                           PRIMS[BZ_CENTER], METRIC_PHYS[GXX], METRIC_PHYS[GXY],
                           METRIC_PHYS[GXZ], METRIC_PHYS[GYY], METRIC_PHYS[GYZ],
                           METRIC_PHYS[GZZ], METRIC_LAP_PSI4[LAPSE]);
              } else if (eos.is_Tabulated) {
                CCTK_VInfo(CCTK_THORNSTRING,
                           "Couldn't find root from: %e %e %e %e %e %e %e, "
                           "rhob approx=%e, rho_b_atm=%e, Bx=%e, By=%e, Bz=%e, "
                           "gij_phys=%e %e %e %e %e %e, alpha=%e",
                           tau_tilde_orig, rho_tilde_orig, S_x_tilde_orig,
                           S_y_tilde_orig, S_z_tilde_orig, Y_e_tilde_orig,
                           ent_tilde_orig,
                           rho_tilde_orig / METRIC_LAP_PSI4[PSI6],
                           eos.rho_b_atm, PRIMS[BX_CENTER], PRIMS[BY_CENTER],
                           PRIMS[BZ_CENTER], METRIC_PHYS[GXX], METRIC_PHYS[GXY],
              ...
              // Increment the number of gridpoints which will need the average
              // fix
              num_of_conservative_averagings_needed++;
            }
          }
          ...
          if (check == 0) {
            ...
            //--------------------------------------------------
            // Enforce limits on primitive variables and recompute
            // conservatives.
            ...
            IllinoisGRMHD_enforce_limits_on_primitives_and_recompute_conservs(
                already_computed_physical_metric_and_inverse, PRIMS, stats, eos,
                METRIC, g4dn, g4up, TUPMUNU, TDNMUNU, CONSERVS);

            rho_tilde[index] = CONSERVS[RHOSTAR];
            S_x_tilde[index] = CONSERVS[STILDEX];
            S_y_tilde[index] = CONSERVS[STILDEY];
            S_z_tilde[index] = CONSERVS[STILDEZ];
            tau_tilde[index] = CONSERVS[TAUENERGY];
            ...
            if (eos.is_Tabulated) {
              ...
              // Conservatives
              Y_e_tilde[index] = CONSERVS[YESTAR];
            }

            // Entropy evolution quantities
            if (eos.evolve_entropy) {
              ent_tilde[index] = CONSERVS[ENTSTAR];
            }
            ...
            // Now we compute the difference between original & new
            // conservatives, for diagnostic purposes:
            error_int_numer += fabs(tau_tilde[index] - tau_tilde_orig) +
                               fabs(rho_tilde[index] - rho_tilde_orig) +
                               fabs(S_x_tilde[index] - S_x_tilde_orig) +
                               fabs(S_y_tilde[index] - S_y_tilde_orig) +
                               fabs(S_z_tilde[index] - S_z_tilde_orig);
            error_int_denom += tau_tilde_orig + rho_tilde_orig + fabs(S_x_tilde_orig) +
                               fabs(S_y_tilde_orig) + fabs(S_z_tilde_orig);

            if (eos.is_Tabulated) {
              error_int_numer += fabs(Y_e_tilde[index] - Y_e_tilde_orig);
              error_int_denom += Y_e_tilde_orig;
            }
            ...
            if (stats.font_fixed == 1)
              font_fixes++;
            if (stats.nan_found == 1) {
              CCTK_VWARN(CCTK_WARN_ALERT,
                         "Found NAN while imposing speed limit");
              nan_found++;
            }
  ...
  if (num_of_conservative_averagings_needed > cons_avgs)
    cons_avgs = num_of_conservative_averagings_needed;
...
if (CCTK_Equals(verbose, "essential") ||
    CCTK_Equals(verbose, "essential+iteration output")) {
  CCTK_VInfo(CCTK_THORNSTRING,
             "C2P: Lev: %d NumPts= %d | Fixes: BU: %d %d %d Font= %d VL= %d "
             "rho*= %d AVG= %d ATM= %d | Failures: %d InHoriz= %d / %d | "
             "Error: %.3e, ErrDenom: %.3e",
             (int)GetRefinementLevel(cctkGH), pointcount, backup1, backup2,
             backup3, font_fixes, vel_limited_ptcount, rho_tilde_fix_applied,
             cons_avgs, atm_resets, failures, failures_inhoriz,
             pointcount_inhoriz, error_int_numer / error_int_denom,
             error_int_denom);
}
if (nan_found) {
  if (GetRefinementLevel(cctkGH) > 6) {
    CCTK_ERROR("Found NAN during con2prim driver. See error messages above. "
               "ABORTING!");
  } else {
    CCTK_VWARN(CCTK_WARN_ALERT, "Found NAN during con2prim driver, but not at "
                                "finest level. Proceeding with caution...");
  }
}

// Very useful con2prim debugger. If the primitives (con2prim) solver fails,
// this will output all data needed to
//     debug where and why the solver failed. Strongly suggested for
//     experimenting with new fixes.
if (((conserv_to_prims_debug == 1) &&
     (error_int_numer / error_int_denom > 0.05)) ||
    (atm_resets != 0)) {

  ofstream myfile;
  char filename[100];
  srand(time(NULL));
  sprintf(filename, "primitives_debug-%e.dat",
  ...
  myfile.write((char *)&igm_eos_table_ceiling_safety_factor,
               1 * sizeof(CCTK_REAL));
  myfile.write((char *)&igm_eos_table_floor_safety_factor,
               1 * sizeof(CCTK_REAL));
  ...
  // Original conservative variables (we used the flux gridfunctions to store
  // these)
  myfile.write((char *)rho_flux, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)tau_flux, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)S_x_flux, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)S_y_flux, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)S_z_flux, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)Y_e_flux, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)ent_flux, (fullsize) * sizeof(CCTK_REAL));

  // Magnetic fields
  myfile.write((char *)Bx_center, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)By_center, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)Bz_center, (fullsize) * sizeof(CCTK_REAL));

  // Primitive variables
  myfile.write((char *)rho, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)press, (fullsize) * sizeof(CCTK_REAL));
  ...
  myfile.write((char *)Y_e, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)temperature, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)eps, (fullsize) * sizeof(CCTK_REAL));
  myfile.write((char *)entropy, (fullsize) * sizeof(CCTK_REAL));
