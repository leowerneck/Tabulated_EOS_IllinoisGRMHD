...
static const int igm_None                 = -1;
static const int igm_Noble2D              = 0;
static const int igm_Noble1D              = 1;
static const int igm_Noble1D_entropy      = 2;
static const int igm_Noble1D_entropy2     = 3;
static const int igm_CerdaDuran2D         = 4;
static const int igm_CerdaDuran3D         = 5;
...
static const int EXTRA_NEWT_ITER
      = 0; /* ZACH SAYS: Original value = 2. But I don't think this parameter > 0 is warranted. Just slows the code for
              no reason, since our tolerances are fine. */
...
void set_cons_from_PRIMS_and_CONSERVS(
      const igm_eos_parameters eos,
      const CCTK_INT c2p_key,
      const CCTK_REAL *restrict METRIC,
      const CCTK_REAL *restrict METRIC_LAP_PSI4,
...
void set_prim_from_PRIMS_and_CONSERVS(
      const igm_eos_parameters eos,
      const CCTK_INT c2p_key,
...
int con2prim_average_neighbor_conservatives(
