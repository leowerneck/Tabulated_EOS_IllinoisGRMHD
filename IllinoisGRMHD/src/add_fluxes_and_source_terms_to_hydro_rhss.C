...
// However, since the metric gfs store the grid point values instead of the cell
// average,
...
#define COMPUTE_FOURMETRIC(g4tt,                                                       \
                           g4tx,                                                       \
                           g4ty,                                                       \
                           g4tz,                                                       \
                           g4xx,                                                       \
    ...
    ({                                                                                 \
        ...
        /* g_{00} = -alpha^2 + beta^i beta^j gamma_{ij} = -alpha^2 + beta^i beta_i     \
         * = -alpha^2 + beta^i g_{0i} */                                               \
...
static void add_fluxes_and_source_terms_to_hydro_rhss(const igm_eos_parameters eos,
                                                      ...
                                                      CCTK_REAL *restrict rho_flux,
                                                      CCTK_REAL *restrict tau_flux,
                                                      CCTK_REAL *restrict S_x_flux,
                                                      CCTK_REAL *restrict S_y_flux,
                                                      CCTK_REAL *restrict S_z_flux,
                                                      CCTK_REAL *restrict Y_e_flux,
                                                      CCTK_REAL *restrict ent_flux,
                                                      CCTK_REAL *restrict rho_rhs,
                                                      CCTK_REAL *restrict tau_rhs,
                                                      CCTK_REAL *restrict S_x_rhs,
                                                      CCTK_REAL *restrict S_y_rhs,
                                                      CCTK_REAL *restrict S_z_rhs,
                                                      CCTK_REAL *restrict Y_e_rhs,
                                                      CCTK_REAL *restrict ent_rhs)
{
    DECLARE_CCTK_PARAMETERS;

    CCTK_REAL dxi[4] = { 1e100, 1.0 / dX[0], 1.0 / dX[1], 1.0 / dX[2] };

    // Notice in the loop below that we go from 3 to cctk_lsh-2 for i, j, AND k,
    // even though
    //   we are only computing the flux in one direction at a time. This is
    //   because in the end, we only need the rhs's from 3 to cctk_lsh-3 for i, j,
    //   and k.
    ...
    for(int k = cctk_nghostzones[2]; k < cctk_lsh[2] - (cctk_nghostzones[2] - 1); k++) {
        for(int j = cctk_nghostzones[1]; j < cctk_lsh[1] - (cctk_nghostzones[1] - 1); j++) {
            for(int i = cctk_nghostzones[0]; i < cctk_lsh[0] - (cctk_nghostzones[0] - 1); i++) {
                ...
                int indexm2 = CCTK_GFINDEX3D(cctkGH,
                                             i - 2 * kronecker_delta[flux_dirn][0],
                                             j - 2 * kronecker_delta[flux_dirn][1],
                                             k - 2 * kronecker_delta[flux_dirn][2]);
                int indexm1 = CCTK_GFINDEX3D(cctkGH,
                                             i - kronecker_delta[flux_dirn][0],
                                             j - kronecker_delta[flux_dirn][1],
                                             k - kronecker_delta[flux_dirn][2]);
                int indexp1 = CCTK_GFINDEX3D(cctkGH,
                                             i + kronecker_delta[flux_dirn][0],
                                             j + kronecker_delta[flux_dirn][1],
                                             k + kronecker_delta[flux_dirn][2]);
                int indexp2 = CCTK_GFINDEX3D(cctkGH,
                                             i + 2 * kronecker_delta[flux_dirn][0],
                                             j + 2 * kronecker_delta[flux_dirn][1],
                                             k + 2 * kronecker_delta[flux_dirn][2]);
                ...
                // Next compute the metric values at the {i,j,k} +/- 1/2 faces (i.e.,
                // the "face values" of the metric)
                CCTK_REAL FACEVAL[NUMVARS_FOR_METRIC_FACEVALS],
                    FACEVALp1[NUMVARS_FOR_METRIC_FACEVALS];
                ...
                CCTK_REAL FACEVAL_LAPSE_PSI4[NUMVARS_METRIC_AUX],
                    FACEVAL_LAPSE_PSI4p1[NUMVARS_METRIC_AUX];
                ...
                //-----------------------------------------------------------------------------
                // If we are not in the ghostzones, then add third-order accurate
                // curvature terms to \tilde{S}_i RHS's
                //    Without this if() statement, _rhs variables are in general set to
                //    nonzero values in ghostzones, which messes up frozen BC's. Also,
                //    this if() statement should speed up the computation slightly.
                   ...
                   && i < cctk_lsh[0] - cctk_nghostzones[0])
                {
                    ...
                    CCTK_REAL g4tt_f, g4tx_f, g4ty_f, g4tz_f, g4xx_f, g4xy_f, g4xz_f, g4yy_f,
                        g4yz_f, g4zz_f;
                    COMPUTE_FOURMETRIC(g4tt_f,
                                       g4tx_f,
                                       g4ty_f,
                                       g4tz_f,
                                       g4xx_f,
                    ...
                    CCTK_REAL g4tt_fp1, g4tx_fp1, g4ty_fp1, g4tz_fp1, g4xx_fp1, g4xy_fp1, g4xz_fp1,
                        g4yy_fp1, g4yz_fp1, g4zz_fp1;
                    COMPUTE_FOURMETRIC(g4tt_fp1,
                                       g4tx_fp1,
                                       g4ty_fp1,
                                       g4tz_fp1,
                                       g4xx_fp1,
                    ...
                    // Needed for S_i_rhs computation:
                    CCTK_REAL S_i_curvature_terms[4] = { 0, 0, 0, 0 };
                    // add \frac{1}{2} \alpha \sqrt{\gamma} T^{\mu \nu} \partial_i g_{\mu
                    // \nu} . Note that i is given by the flux direction.
                    //   (Source term of Eq 43 in
                    //   http://arxiv.org/pdf/astro-ph/0503420.pdf)
                    S_i_curvature_terms[flux_dirn]
                    ...
                    //   (Last part of Eq. 39 source term in
                    //   http://arxiv.org/pdf/astro-ph/0503420.pdf)
                    ...
                    // \partial_t \tilde{S}_i = - \partial_i (\alpha \sqrt{\gamma} T^j_i)
                    // + \frac{1}{2}\alpha \sqrt{\gamma} T^{\mu \nu}g_{\mu \nu,i} Notice
                    // that S_i_curvature_terms[N]=0 for N!=flux_dirn.
                    S_x_rhs[index] += S_i_curvature_terms[1];
                    S_y_rhs[index] += S_i_curvature_terms[2];
                    S_z_rhs[index] += S_i_curvature_terms[3];
                }
            }
        }
    }
    ...
    for(int k = cctk_nghostzones[2]; k < cctk_lsh[2] - cctk_nghostzones[2]; k++) {
        for(int j = cctk_nghostzones[1]; j < cctk_lsh[1] - cctk_nghostzones[1]; j++) {
            for(int i = cctk_nghostzones[0]; i < cctk_lsh[0] - cctk_nghostzones[0]; i++) {
                ...
                rho_rhs[index] += (rho_flux[index] - rho_flux[indexp1]) * dxi[flux_dirn];
                tau_rhs[index] += (tau_flux[index] - tau_flux[indexp1]) * dxi[flux_dirn];
                S_x_rhs[index] += (S_x_flux[index] - S_x_flux[indexp1]) * dxi[flux_dirn];
                S_y_rhs[index] += (S_y_flux[index] - S_y_flux[indexp1]) * dxi[flux_dirn];
                S_z_rhs[index] += (S_z_flux[index] - S_z_flux[indexp1]) * dxi[flux_dirn];
                if(eos.is_Tabulated) {
                    Y_e_rhs[index] += (Y_e_flux[index] - Y_e_flux[indexp1]) * dxi[flux_dirn];
                }
                if(eos.evolve_entropy) {
                    ent_rhs[index] += (ent_flux[index] - ent_flux[indexp1]) * dxi[flux_dirn];
                }
            }
        }
    }
