...
static void
GRHayL_add_fluxes_and_source_terms_to_hydro_rhss(
    const igm_eos_parameters eos,
    const int flux_dirn,
    const int dirlength,
    const int ghostzone,
  ...
  for(int k = ghostzone; k < dirlength - (ghostzone - 1); k++) {
    for(int j = ghostzone; j < dirlength - (ghostzone - 1); j++) {
      for(int i = ghostzone; i < dirlength - (ghostzone - 1); i++) {
        ...
        // Next set metric on the faces, applying a 3rd-order lopsided stencil.
        int indexm2 = indexf(
            dirlength, i - 2 * kronecker_delta[flux_dirn][0],
            j - 2 * kronecker_delta[flux_dirn][1], k - 2 * kronecker_delta[flux_dirn][2]
        );
        int indexm1 = indexf(
            dirlength, i - kronecker_delta[flux_dirn][0], j - kronecker_delta[flux_dirn][1],
            k - kronecker_delta[flux_dirn][2]
        );
        int indexp1 = indexf(
            dirlength, i + kronecker_delta[flux_dirn][0], j + kronecker_delta[flux_dirn][1],
        ...
        if(k < dirlength - ghostzone && j < dirlength - ghostzone && i < dirlength - ghostzone) {
          ...
          // add - ( T^{00} \beta^i + T^{0i} ) \partial_i \alpha.
          //   (Last part of Eq. 39 source term in http://arxiv.org/pdf/astro-ph/0503420.pdf)
          const CCTK_REAL alpha_sqrtgamma = 2.0 * half_alpha_sqrtgamma;
          const CCTK_REAL tau_source
              = alpha_sqrtgamma
                * (-(TUP[0][0] * METRIC[SHIFTX + (flux_dirn - 1)] + TUP[0][flux_dirn])
                   * lapse_deriv[flux_dirn]);
          const CCTK_REAL st_x_source = st_i_curvature_terms[1];
          const CCTK_REAL st_y_source = st_i_curvature_terms[2];
          const CCTK_REAL st_z_source = st_i_curvature_terms[3];

          // Eq 43 in http://arxiv.org/pdf/astro-ph/0503420.pdf:
          // \partial_t \tilde{S}_i = - \partial_i (\alpha \sqrt{\gamma} T^j_i) + \frac{1}{2}\alpha
          // \sqrt{\gamma} T^{\mu \nu}g_{\mu \nu,i} Notice that st_i_curvature_terms[N]=0 for
          // N!=flux_dirn.
          tau_rhs[index] += tau_source;
          st_x_rhs[index] += st_x_source;
          st_y_rhs[index] += st_y_source;
          st_z_rhs[index] += st_z_source;

          if(index == 5272) {
            printf(
                "F : %22.15e %22.15e %22.15e %22.15e %22.15e %22.15e %22.15e\n",
                rho_star_flux[index], tau_flux[index], st_x_flux[index], st_y_flux[index],
                st_z_flux[index], Ye_star_flux[index], S_star_flux[index]
            );
            // printf("S : %22.15e %22.15e %22.15e %22.15e\n", tau_source, st_x_source,
            // st_y_source, st_z_source);
          }
        }
      }
    }
  ...
  for(int k = ghostzone; k < dirlength - ghostzone; k++) {
    for(int j = ghostzone; j < dirlength - ghostzone; j++) {
      for(int i = ghostzone; i < dirlength - ghostzone; i++) {
        ...
        if(index == 5272) {
          printf(
              "Rb: %22.15e %22.15e %22.15e %22.15e %22.15e %22.15e %22.15e\n", rho_star_rhs[index],
              tau_rhs[index], st_x_rhs[index], st_y_rhs[index], st_z_rhs[index],
              Ye_star_rhs[index], S_star_rhs[index]
          );
        }
        ...
        if(index == 5272) {
          printf(
              "Ra: %22.15e %22.15e %22.15e %22.15e %22.15e %22.15e %22.15e\n", rho_star_rhs[index],
              tau_rhs[index], st_x_rhs[index], st_y_rhs[index], st_z_rhs[index],
              Ye_star_rhs[index], S_star_rhs[index]
          );
        }
