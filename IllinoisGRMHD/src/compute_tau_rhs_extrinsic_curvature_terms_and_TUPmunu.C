...
static void GRHayL_compute_tau_rhs_extrinsic_curvature_terms_and_TUPmunu( const igm_eos_parameters eos,
                                                                          const int dirlength,
                                                                          const int ghostzone,
                                                                          CCTK_REAL *restrict dX,
                                                                          CCTK_REAL **metric,
  ...
  for(int k=ghostzone; k<dirlength-(ghostzone-1); k++)
    for(int j=ghostzone; j<dirlength-(ghostzone-1); j++)
      for(int i=ghostzone; i<dirlength-(ghostzone-1); i++) {
        ...
        // printf("Extrinsic curvature is fine\n");
        ...
        // printf("Metric is fine\n");

        // The "vector" U represents the primitive variables
        CCTK_REAL U[MAXNUMVARS];
        for(int ii=0;ii<MAXNUMVARS;ii++) U[ii] = prims[ii].gf[index];

        // printf("prims is fine\n");
        ...
        //    Without this if() statement, _rhs variables are in general set to nonzero values in ghostzones, which messes up frozen BC's.
        //    Also, this if() statement should speed up the computation slightly.
        // printf("about to check the if statement\n");
