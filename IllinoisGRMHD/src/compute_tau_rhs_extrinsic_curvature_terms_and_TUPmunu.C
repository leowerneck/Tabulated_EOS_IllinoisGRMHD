static void
compute_tau_rhs_extrinsic_curvature_terms_and_TUPmunu(const igm_eos_parameters eos,
...
{
    // These loop extents must be consistent with add_fluxes_and_source_terms_to_hydro_rhss(),
    // since we use TUPmunu there as well.
        ...
        for(int j = cctk_nghostzones[1]; j < cctk_lsh[1] - (cctk_nghostzones[1] - 1); j++) {
            for(int i = cctk_nghostzones[0]; i < cctk_lsh[0] - (cctk_nghostzones[0] - 1); i++) {
                ...
                for(int ii = 0; ii < MAXNUMVARS; ii++) {
                    U[ii] = prims[ii].gf[index];
                }

                struct output_stats stats;
                stats.failure_checker = 0;
                CCTK_REAL u0L;
                impose_speed_limit_output_u0(METRIC,
                                             U,
                                             METRIC_LAP_PSI4[PSI4],
                                             METRIC_LAP_PSI4[LAPSEINV],
                                             stats,
                                             u0L);
                ...
                compute_smallba_b2_and_u_i_over_u0_psi4(METRIC,
                                                        METRIC_LAP_PSI4,
                                                        U,
                                                        u0L,
                                                        ONE_OVER_LAPSE_SQRT_4PI,
                                                        u_x_over_u0_psi4,
                                                        u_y_over_u0_psi4,
                                                        u_z_over_u0_psi4,
                                                        smallb);
                ...
                g4up[1][1] = METRIC[GUPXX] * Psim4
                           - ONE_OVER_LAPSE_SQUARED * METRIC[SHIFTX] * METRIC[SHIFTX];
                // Note that for i!=j, gupij is not stored in METRIC, since we don't need it in
                // face value calculations.
                g4up[1][2] = gupxy[index] * Psim4
                           - ONE_OVER_LAPSE_SQUARED * METRIC[SHIFTX] * METRIC[SHIFTY];
                g4up[1][3] = gupxz[index] * Psim4
                           - ONE_OVER_LAPSE_SQUARED * METRIC[SHIFTX] * METRIC[SHIFTZ];
                g4up[2][2] = METRIC[GUPYY] * Psim4
                           - ONE_OVER_LAPSE_SQUARED * METRIC[SHIFTY] * METRIC[SHIFTY];
                g4up[2][3] = gupyz[index] * Psim4
                           - ONE_OVER_LAPSE_SQUARED * METRIC[SHIFTY] * METRIC[SHIFTZ];
                g4up[3][3] = METRIC[GUPZZ] * Psim4
                           - ONE_OVER_LAPSE_SQUARED * METRIC[SHIFTZ] * METRIC[SHIFTZ];

                // Next compute T^{\mu \nu}:
                // (Eq. 33 in http://arxiv.org/pdf/astro-ph/0503420.pdf):
                // T^{mn} = (rho_0 h + b^2) u^m u^n + (P + 0.5 b^2) g^{mn} - b^m b^n, where m and n
                // both run from 0 to 3.
                CCTK_REAL TUP[4][4];
                for(int ii = 0; ii < 4; ii++) {
                    for(int jj = ii; jj < 4; jj++) {
                        TUP[ii][jj] = rho0_h_plus_b2 * uUP[ii] * uUP[jj]
                                    + P_plus_half_b2 * g4up[ii][jj]
                                    - smallb[SMALLBT + ii] * smallb[SMALLBT + jj];
                    }
                }
                //-----------------------------------------------------------------------------

                //-----------------------------------------------------------------------------
                // If we are not in the ghostzones, then compute extrinsic curvature terms for
                // tau_rhs:
                //    Without this if() statement, _rhs variables are in general set to nonzero
                //    values in ghostzones, which messes up frozen BC's. Also, this if() statement
                //    should speed up the computation slightly.
                if(k < cctk_lsh[2] - cctk_nghostzones[2] && j < cctk_lsh[1] - cctk_nghostzones[1]
                   && i < cctk_lsh[0] - cctk_nghostzones[0])
                {
                    //  \alpha \sqrt{\gamma} (T^{00} \beta^i \beta ^j + 2T^{0i} \beta^j +
                    //  T^{ij})*K_{ij}
                    CCTK_REAL alpha_sqrtgamma = 2.0 * half_alpha_sqrtgamma;

                    //  \alpha \sqrt{\gamma} (T^{00} \beta^i \beta ^j + 2T^{0i} \beta^j +
                    //  T^{ij})*K_{ij}
                    CCTK_REAL tau_rhs_extrinsic_curvature_terms
                        = alpha_sqrtgamma
                        * (TUP[0][0]
                               * (SQR(METRIC[SHIFTX]) * KxxL + SQR(METRIC[SHIFTY]) * KyyL
                                  + SQR(METRIC[SHIFTZ]) * KzzL
                                  + 2.0
                                        * (METRIC[SHIFTX] * METRIC[SHIFTY] * KxyL
                                           + METRIC[SHIFTX] * METRIC[SHIFTZ] * KxzL
                                           + METRIC[SHIFTY] * METRIC[SHIFTZ] * KyzL))
                           + 2.0
                                 * (TUP[0][1] * METRIC[SHIFTX] * KxxL
                                    + TUP[0][2] * METRIC[SHIFTY] * KyyL
                                    + TUP[0][3] * METRIC[SHIFTZ] * KzzL
                                    + (TUP[0][1] * METRIC[SHIFTY] + TUP[0][2] * METRIC[SHIFTX])
                                          * KxyL
                                    + (TUP[0][1] * METRIC[SHIFTZ] + TUP[0][3] * METRIC[SHIFTX])
                                          * KxzL
                                    + (TUP[0][2] * METRIC[SHIFTZ] + TUP[0][3] * METRIC[SHIFTY])
                                          * KyzL)
                ...
                for(int ii = 0; ii < 4; ii++) {
                    for(int jj = ii; jj < 4; jj++) {
                        TUPmunu[counter][index] = TUP[ii][jj];
                        counter++;
                    }
                }
            }
