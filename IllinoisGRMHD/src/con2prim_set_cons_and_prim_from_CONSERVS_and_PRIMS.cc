...
void set_cons_from_PRIMS_and_CONSERVS(
  ...
  CCTK_REAL uu = -CONSERVS[TAUENERGY] * METRIC_LAP_PSI4[LAPSE] - (METRIC_LAP_PSI4[LAPSE] - 1.0) * CONSERVS[RHOSTAR]
                 + METRIC[SHIFTX] * CONSERVS[STILDEX] + METRIC[SHIFTY] * CONSERVS[STILDEY]
                 + METRIC[SHIFTZ] * CONSERVS[STILDEZ]; // note the minus sign on tau
  ...
  //--------------- Entropy ----------------
  //----------------------------------------
  if((c2p_key == igm_Noble1D_entropy) || (c2p_key == igm_Noble1D_entropy2) || (c2p_key == igm_Palenzuela1D)
...
void set_prim_from_PRIMS_and_CONSERVS(
  ...
  if((c2p_key == igm_Noble2D) || (c2p_key == igm_Noble1D) || (c2p_key == igm_Noble1D_entropy)
     || (c2p_key == igm_Noble1D_entropy2) || (c2p_key == igm_CerdaDuran2D) || (c2p_key == igm_CerdaDuran3D)) {
