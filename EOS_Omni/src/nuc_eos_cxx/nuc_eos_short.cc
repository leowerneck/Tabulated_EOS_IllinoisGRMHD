...
extern "C"
  ...
  extern "C"
  void CCTK_FNAME(nuc_eos_m_kt3_short)(const int *restrict n_in,
                                       const double *restrict rho, 
                                       double *restrict temp,
                                       const double *restrict ye,
                                       double *restrict eps,
                                       const double *restrict prs,
                                       double *restrict ent,
                                       double *restrict cs2,
                                       double *restrict dedt,
                                       double *restrict dpderho,
                                       ...
                                       int *restrict anyerr)
{
  ...
  *anyerr = 0;
  ...
  for(int i=0;i<n;i++) {
    ...
    const double lxprs = log(prs[i]);
#if DEBUG
      fprintf(stderr,"%d %15.6E %15.6E %15.6E %15.6E\n",i,lr,lt,ye[i],lxprs);
      fprintf(stderr,"%d %15.6E %15.6E %15.6E %15.6E\n",i,
	      exp(lr),exp(lt),ye[i],exp(lxprs));
#endif

      nuc_eos_findtemp_press(lr,lt,ye[i],lxprs,*prec,
                             (double *restrict)(&ltout),&keyerr[i]);
