...
extern "C" void
set_IllinoisGRMHD_metric_GRMHD_variables_based_on_HydroBase_and_ADMBase_variables(CCTK_ARGUMENTS)
{
    ...
    if(rho_b_atm > 1e199) {
        CCTK_VError(VERR_DEF_PARAMS,
                    "You MUST set rho_b_atm to some reasonable value in your param.ccl file.\n");
    }

    // Convert ADM variables (from ADMBase) to the BSSN-based variables expected by this routine.
    IllinoisGRMHD_convert_ADM_to_BSSN__enforce_detgtij_eq_1__and_compute_gtupij(cctkGH,
                                                                                cctk_lsh,
                                                                                gxx,
                                                                                gxy,
                                                                                gxz,
...
#pragma omp parallel for
    ...
    if(ID_converter_ILGRMHD_pressure_depletion_factor != 0.0) {
        apply_pressure_depletion = true;
    }
            ...
            for(int i = 0; i < cctk_lsh[0]; i++) {
                ...
                if(eos.is_Hybrid) {
                    if(apply_pressure_depletion) {
                        ...
                        const CCTK_REAL pressL
                            = (1.0 - ID_converter_ILGRMHD_pressure_depletion_factor)
                            * press[index];
                        // Recompute P and eps
                        const CCTK_INT ppidx
                            = find_polytropic_K_and_Gamma_index_from_P(eos, pressL);
                ...
                A_x_tilde[index] = Avec[CCTK_GFINDEX4D(cctkGH, i, j, k, 0)];
                A_y_tilde[index] = Avec[CCTK_GFINDEX4D(cctkGH, i, j, k, 1)];
                A_z_tilde[index] = Avec[CCTK_GFINDEX4D(cctkGH, i, j, k, 2)];
                Phi_tilde[index] = Aphi[index];
    ...
    for(int k = 0; k < cctk_lsh[2]; k++) {
        for(int j = 0; j < cctk_lsh[1]; j++) {
            for(int i = 0; i < cctk_lsh[0]; i++) {
                ...
                Phi_tilde[index] *= one_plus_pert;
                A_x_tilde[index] *= one_plus_pert;
                A_y_tilde[index] *= one_plus_pert;
                A_z_tilde[index] *= one_plus_pert;
            }
        }
    }
    ...
    for(int k = 0; k < cctk_lsh[2]; k++) {
        for(int j = 0; j < cctk_lsh[1]; j++) {
            for(int i = 0; i < cctk_lsh[0]; i++) {
                ...
                int shiftedim1
                    = (i - 1) * (i != 0); // This way, i=0 yields shiftedim1=0 and shiftedi=1, used
                                          // below for our COPY boundary condition.
                ...
                // "Grid" A_z_tilde(i,j,k) is actually A_y_tilde(i+1/2,j+1/2,k)
                ...
                // Now multiply Bx and Bx_stagger by 1/sqrt(gamma(i+1/2,j,k)]) = 1/sqrt(1/2 [gamma
                // + gamma_ip1]) = exp(-6 x 1/2 [phi + phi_ip1] )
                int imax_minus_i = (cctk_lsh[0] - 1) - i;
                int indexip1jk
                    = CCTK_GFINDEX3D(cctkGH, i + ((imax_minus_i > 0) - (0 > imax_minus_i)), j, k);
                ...
                // Now multiply By and By_stagger by 1/sqrt(gamma(i,j+1/2,k)]) = 1/sqrt(1/2 [gamma
                // + gamma_jp1]) = exp(-6 x 1/2 [phi + phi_jp1] )
                int jmax_minus_j = (cctk_lsh[1] - 1) - j;
                int indexijp1k
                    = CCTK_GFINDEX3D(cctkGH, i, j + ((jmax_minus_j > 0) - (0 > jmax_minus_j)), k);
                ...
                int indexijkp1
                    = CCTK_GFINDEX3D(cctkGH, i, j, k + ((kmax_minus_k > 0) - (0 > kmax_minus_k)));
    ...
    for(int k = 0; k < cctk_lsh[2]; k++) {
        for(int j = 0; j < cctk_lsh[1]; j++) {
            for(int i = 0; i < cctk_lsh[0]; i++) {
                ...
                IllinoisGRMHD_enforce_limits_on_primitives_and_recompute_conservs(zero_int,
                                                                                  PRIMS,
                                                                                  stats,
                                                                                  eos,
                                                                                  METRIC,
